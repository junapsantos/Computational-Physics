...
//Nota: 
//Falta ler ficheiro a partir da linha em que tem *'s
//DataReader, filename != nullptr, será nullptr?!
//Sugestão professor: Sugere-se assim que o armazenamento de valores e a análise seja feita a partir do ano de 1849 (inclusive
//linha 27 main, if(!DataReader::FileExists("SunspotNumberDATA2020.txt")) exit(1);
//DataManip::~DataManip()
...
void main_trab01() {
  ...
  cout << "number of data points read: " << V.size() << endl;
  ...
  for (auto const& x: Vsort ) {
      cout << c << " | " << "(" << std::setprecision(7) << x.first << ", " << x.second << ")" << endl;
      c++;
      if (c == 10)break;
  }
