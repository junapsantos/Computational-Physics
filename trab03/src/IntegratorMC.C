...
int IntegratorMC::GetN(){
  return NSamp;
}

void IntegratorMC::UniformRandom(double &res, double &erro, int NSamples){
    NSamp = NSamples;
    TRandom3 rgen(0);
    double a;
...
void IntegratorMC::UniformRandom(double inicio, double final, double &res, double &erro, int NSamples){
    NSamp = NSamples;
    TRandom3 rgen(0);
    double a;
    double res_quadrado = 0;
    ...
    for(int i = 0; i < NSamples; i++){
    a = x + rgen.Uniform() * (final - inicio); 
    func = F -> Eval(a);
    res += func;
    res_quadrado += func * func;
    err = sqrt(-((res/NSamp)*(res/NSamp) - (res_quadrado/NSamp))/NSamp);
    err = err * (final - inicio);
    }
    res = res * (final - inicio);
    res = res/(NSamp);
