	...
	/*if(M1.Determinant() == 0){
		...
		for(int bn = 0; bn < n; bn++){
			...
			for(int i=0; i<n; i++){
   			...
   			b[bn]=M1[bn][n];
			M1[bn].SetEntries(M1.GetRowN(),array1);
			delete [] array1;
		}
...
Vec EqSolver::GaussEliminationSolver(){
	...
	Vec solucao(M1.GetColN());

	for(int i = M1.GetColN() - 1; i > -1 ; i --){
		solucao[i] = b[i] / M1[i].At(i);

		for(int j = M1.GetColN() - 1; j > i; j --)
			solucao[i] = solucao[i] - (M1[i].At(j)*solucao[i]/M1[i].At(i));
	}

	return solucao;
}

void EqSolver::LUdecomposition(FCmatrixFull& M1){
...
void EqSolver::Print(){
    ...
    for(int i=0; i<M1.GetRowN(); i++)
    {
        ...
        {
            cout << setw(8) << std::right << std::setprecision(3) << std::fixed << M1[i].At(j)<< fixed<<setw(8);
        }
