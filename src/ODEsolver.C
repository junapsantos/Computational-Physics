...
vector<ODEPoint> ODEsolver::RK4_Projetil(ODEPoint iPoint, double step, double T, double*par){
 
	int Nstep = 1 + int(T/step); 
		...
		for (int i = 0; i <Nstep; ++i) {
			...
			vP[i+1]=iPoint; 												//para kinda alocar espaço
		 	...
		 	for(int j=0; j<N; j++){
		 		...
		 		/////K1
			 	K1 = step*F[j].EvalPar(vP[i].Get_T_X_ptr(), par);				//K1 = step*f(ti,yi)

			 	////K2
			 	vK2[0] = vP[i].T() + step*0.5;								//t(i+1/2) = ti + 1/2step
			 	for(int k=1; k<=N; k++)										//y(i+1/2) = yi + (K1)/2
			 		vK2[k] = vP[i].X(k-1) + K1*0.5;
			 	K2= step*F[j].EvalPar(vK2,par);									//K2 = step * f( t(i+1/2) , y(i+1/2) )

			 	////K3
			 	vK3[0]=vK2[0];												//t(i+1/2) = ti + 1/2step
			 	for(int k=1; k<=N; k++)										//y(i+1/2) = yi + (K2)/2
			 		vK3[k] = vP[i].X(k-1) + K2*0.5;
			 	K3= step*F[j].EvalPar(vK3,par);

			 	////K4
			 	vK4[0]= vP[i].T() + step;									//t(i+1/2) = ti + step
			 	for(int k=1; k<=N; k++)										//y(i+1/2) = yi + K3
			 		vK4[k] = vP[i].X(k-1) + K3;
			 	K4= step*F[j].EvalPar(vK4,par);									//K2 = step * f( t(i+1/2) , y(i+1/2) )
		 	...
		 	if(stop<=10e-2){												//se o y for 0 para
		 		for(int istop=i+1;istop <Nstep; ++istop) {
		 			vP[istop+1]=iPoint; 
		 			vP[istop+1].SetT(vP[istop].T() + step); 
		 			vP[istop+1].SetV(nextv);
		 			vP[istop+1].SetX(2,0);
		 			vP[istop+1].SetX(3,0);
		 		}											
